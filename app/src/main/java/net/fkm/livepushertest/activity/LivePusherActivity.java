...
import net.fkm.livepushertest.utils.HttpHelper;
...
public class LivePusherActivity extends BaseActivity {
    ...
    public void startPush() {
        String streamName = ShareUtils.getString(this, "StreamName", "live");


        String time = BaseTools.dateToStamp();
        // 第一个参数传递流的名称（相对于直播时的房间号），第二个参数传递过期十位数的时间戳
        String pushUrl = AliyunPushUtils.CreatePushUrl(streamName, time);
        // 传递的参数值必须和推流的一致，第一个参数传递流的名称（相对于直播时的房间号），第二个参数传递过期十位数的时间戳
        String liveUrl = AliyunPushUtils.GetPlayUrl(streamName, time);
        String jsonInfo = "{\"stream_name\": \"" + streamName + "\", \"live_url\": \"" + liveUrl + "\"}";

        L.i("获取推流地址：" + pushUrl);
        L.i("获取拉流地址：\n" + liveUrl);
        L.i("#################");
        String info = sendHttp(jsonInfo, "/live/add");
//        发送http请求
        ...
        ToastUtil.showToast("已开始直播");
    }

    public String sendHttp(String jsonInfo, String method) {
        String info = HttpHelper.sendJsonPost(jsonInfo, jsonInfo);
        return info;
    }
    ...
    public void stopPush() {
        ...
        try {
            ...
            String jsonInfo = "{\"stream_name\": \"" + streamName + "\"}";
            sendHttp(jsonInfo, "/live/delete");
